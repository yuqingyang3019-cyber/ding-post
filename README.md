...
push 到 `main` 后，GitHub Actions 会使用 Python 3.9 执行语法检查，将 Python 3.9 兼容的运行依赖安装到 `.python/` 并随代码包部署，然后使用 Serverless Devs 基于 `s.yaml` 部署到 FC3。部署地域已固定为华东 1（杭州）：`cn-hangzhou`。
